...
exports.exceptionHandler = (err, req, res, next) => {
  ...
  if (err.statusCode === 400) {
    ...
    return res.status(400).json(new Response('Failed', err.message));
  }
  if (err.statusCode === 404) {
    res.statusMessage = err.statusMessage || 'Requested URL not found';

    return res.status(404).json(new Response('Failed', err.message));
  }

  res.statusMessage = err.statusMessage || 'An internal server error occurred';

  return res.status(500).json(new Response('Failed', err.message));
};